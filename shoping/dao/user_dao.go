...
func (ud *UserDao) QueryUserInfoById(id int64) (model.User, error) {
	...
	err = row.Scan(&user.Id, &user.Username, &user.Password, &user.Email, &user.Phone, &user.Salt, &user.Avatar, &user.Statement, &user.Gender, &user.Balance)
	if err != nil {
		log.Fatal(err.Error())
		return user, err
	}
...
func (ud *UserDao) InsertUser(user model.User) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(user.Id, user.Username, user.Password, user.Phone, user.Salt, user.Balance)
	if err != nil {
		log.Fatal(err.Error())
		return err
	}
